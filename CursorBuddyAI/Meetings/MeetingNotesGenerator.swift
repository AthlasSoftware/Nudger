...
class MeetingNotesGenerator {
    ...
    func exportToPages(notes: MeetingNotes) async throws -> URL {
        ...
        // Open in default app (Pages if installed, otherwise TextEdit)
        let opened = NSWorkspace.shared.open(fileURL)
        if !opened {
            Log.app.warning("Failed to open file: \(fileURL.path)")
        }
    ...
    private func buildNotesPrompt(transcript: String, meetingTitle: String) -> String {
        ...
        CRITICAL RULES:
        - ONLY extract information that is ACTUALLY in the transcript
        - If the transcript is too short or unclear, use EMPTY ARRAYS []
        - Do NOT make up or hallucinate information
        - Do NOT add example or placeholder content
        - If you can't identify something, leave that array EMPTY
        - Summary should reflect what was actually discussed (or "No substantial discussion captured" if silent)
        - Key points should capture main discussion topics (or [] if none)
        - Action items should be specific and actionable (or [] if none mentioned)
        - Decisions should be clear outcomes or agreements (or [] if none made)
        - Participants should list people mentioned or speaking (or [] if none identified)
    ...
    private func generateRTF(from notes: MeetingNotes) -> String {
        ...
        // RTF font sizes: fs24 = 12pt, fs28 = 14pt, fs32 = 16pt, fs48 = 24pt
        // RTF uses escaped Unicode: \u8226 = bullet, \u9745 = checkbox, \u10003 = checkmark
        var rtf = """
        {\\rtf1\\ansi\\deff0
        {\\fonttbl{\\f0 Helvetica;}{\\f1 Helvetica-Bold;}}
        {\\colortbl;\\red0\\green0\\blue0;\\red80\\green80\\blue80;\\red100\\green100\\blue255;}
        
        {\\f1\\fs48\\cf3 \(escapeRTF(notes.session.title))\\par}
        \\par
        {\\f0\\fs22\\cf2 \(escapeRTF(startTime)) \\u8226  \(duration)\\par}
        \\par
        \\line
        \\par
        
        {\\f1\\fs32 Meeting Overview\\par}
        {\\f0\\fs24 \(escapeRTF(notes.summary))\\par}
        \\par
        ...
        if !notes.participants.isEmpty {
            ...
            for participant in notes.participants {
                rtf += "\\u8226  \(escapeRTF(participant))\\par\n"
            }
            rtf += "}\\par\n"
        }
        ...
        if !notes.keyPoints.isEmpty {
            ...
            {\\f0\\fs24
            """
            for point in notes.keyPoints {
                rtf += "\\u8226  \(escapeRTF(point))\\par\n"
            }
            rtf += "}\\par\n"
        }
        ...
        rtf += """
        \\par
        {\\f0\\fs20\\cf2 Generated by Nudger \\u8226  \(escapeRTF(dateFormatter.string(from: Date())))\\par}
        }
        """
